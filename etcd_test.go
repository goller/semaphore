...
const SemaphorePrefix = "/TestKey/semaphore"
...
func TestEtcdLockClientInit(t *testing.T) {
	for i, tt := range []struct {
		...
		{nil, false, SemaphorePrefix, SemaphorePrefix},
		...
		{nil, false, "prod/database", "prod/database"},
	} {
		elc, got := NewEtcdLockClient(context.Background(), &testEtcdClient{err: tt.ee}, tt.group)
		if (got != nil) != tt.want {
...
func TestEtcdLockClientGet(t *testing.T) {
	for i, tt := range []struct {
		...
		gs, ge := elc.Get(context.Background())
		if tt.we {
			if ge == nil {
...
func TestEtcdLockClientSet(t *testing.T) {
	for i, tt := range []struct {
		...
		got := elc.Set(context.Background(), tt.sem)
		if (got != nil) != tt.want {
